...
print("-" * 60)
try:
    response = client.chat.completions.create(model="gpt-5.1", messages=[{"role": "user", "content": question}])
    ...
    if hasattr(response, "output") and response.output:
        for i, item in enumerate(response.output):
            ...
            if item_type == "ResponseTextItem":
                ...
                print("Reasoning found (unexpected!)")
            elif item_type == "ResponseOutputMessage":
                if hasattr(item, "content"):
                    for content_item in item.content:
                        if hasattr(content_item, "text"):
...
try:
    response = client.chat.completions.create(
        model="gpt-5.1-chat-latest", messages=[{"role": "user", "content": question}]
    )
